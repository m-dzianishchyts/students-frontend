...
import { BehaviorSubject, lastValueFrom, mergeMap, Observable, Subject } from "rxjs";
...
export class BackEndService {
    ...
    fetchUserGroups(): Observable<Group[]> {
        console.log("Fetching groups...");
        const user = this.userSubject.getValue();
        if (!user) {
            throw new Error("User is undefined!");
        }
        return this.http.get<Group[]>(`${environment.apiUrl}/users/${user.id}/groups`, { withCredentials: true });
    }
    ...
    fetchGroupQueuesPerspective() {
        ...
        return this.http.get<PerspectiveQueue[]>(`${environment.apiUrl}/groups/${group.id}/queues/perspective/${user.id}`, {
    ...
    fetchGroupUsers(): Observable<User[]> {
        ...
        return this.http.get<User[]>(`${environment.apiUrl}/groups/${group.id}/members`, { withCredentials: true });
    }
    ...
    fetchQueueUsers(queueId: string | undefined): Observable<User[]> {
        ...
        return this.http.get<User[]>(`${environment.apiUrl}/queues/${neededId}/members`, {
            withCredentials: true,
        });
    }
    ...
    createQueue(queueInfo: QueueInfo, groupId?: string) {
        ...
        return this.requestQueueCreate(neededGroupId, queueInfo).subscribe(() => this.refreshGroup());
    }

    createGroup(groupInfo: GroupInfo) {
        return this.requestGroupCreate(groupInfo).pipe(mergeMap(() => this.fetchUserGroups()));
    }
    ...
    refreshGroupByQueue(queueId: string): void {
    ...
    refreshQueue(queueId?: string): void {
        const queue = this.queueSubject.getValue();
    ...
    fetchGroupByQueue(queueId: string): Observable<Group> {
        ...
        return this.http.get<Group>(`${environment.apiUrl}/queues/${queueId}/group`, {
            withCredentials: true,
        });
    }
    ...
    addUserToGroup(userEmail: string) {
        ...
        this.requestGroupAddUser(group.id, userEmail).subscribe(() => this.refreshGroup());
    }
    ...
    private fetchUser() {
        ...
        return this.http.get<User>(`${environment.apiUrl}/users/token`, { withCredentials: true });
    }

    private fetchGroup(groupId: string) {
        console.log("Fetching group...");
        return this.http.get<Group>(`${environment.apiUrl}/groups/${groupId}`, { withCredentials: true });
    }

    private fetchQueue(queueId: string) {
        console.log("Fetching queue...");
        return this.http.get<Queue>(`${environment.apiUrl}/queues/${queueId}`, { withCredentials: true });
    }

    private fetchQueueMember(queueId: string, userId: string) {
        console.log("Fetching queue member...");
        return this.http.get<LightWeightQueueMember>(`${environment.apiUrl}/queues/${queueId}/members/${userId}`, {
            withCredentials: true,
        });
    }
    ...
    private requestQueueCreate(groupId: string, queueInfo: QueueInfo) {
        ...
        return this.http.post<Queue>(`${environment.apiUrl}/groups/${groupId}/queues`, queueInfo, { withCredentials: true });
    }

    private requestQueueShuffle(queueId: string) {
        console.log("Shuffling queue...");
        return this.http.post<Queue>(`${environment.apiUrl}/queues/${queueId}/shuffle`, null, { withCredentials: true });
    }

    private requestQueueRotate(queueId: string) {
        console.log("Rotating queue...");
        return this.http.post<Queue>(`${environment.apiUrl}/queues/${queueId}/rotate`, null, { withCredentials: true });
    }

    private requestQueueAddMember(queueId: string, userId: string) {
        console.log("Adding queue member...");
        return this.http.put<QueueMember>(`${environment.apiUrl}/queues/${queueId}/members/${userId}`, null, { withCredentials: true });
    }

    private requestQueueRemoveMember(queueId: string, userId: string) {
        console.log("Removing queue member...");
        return this.http.delete<void>(`${environment.apiUrl}/queues/${queueId}/members/${userId}`, { withCredentials: true });
    }

    private requestQueueMemberStatusUpdate(queueId: string, userId: string, status: boolean) {
        console.log("Setting member status...");
        return this.http.patch<{ id: string; status: boolean }>(
    ...
    private requestQueueDelete(groupId: string, queueId: string) {
        ...
        return this.http.delete<void>(`${environment.apiUrl}/groups/${groupId}/queues/${queueId}`, { withCredentials: true });
    }

    private requestGroupCreate(groupInfo: GroupInfo) {
        console.log("Creating group...");
        return this.http.post<Group>(`${environment.apiUrl}/groups`, groupInfo, { withCredentials: true });
    }

    private requestGroupAddUser(groupId: string, userEmail: string) {
        console.log("Adding user to group...");
        return this.http.put<void>(
            `${environment.apiUrl}/groups/${groupId}/members/email`,
            { email: userEmail },
            { withCredentials: true }
        );
    }
    ...
    private requestGroupRemoveMember(groupId: string, userId: string) {
        ...
        return this.http.delete<void>(`${environment.apiUrl}/groups/${groupId}/members/${userId}`, { withCredentials: true });
    }

    private requestGroupDelete(groupId: string) {
        console.log("Deleting group...");
        return this.http.delete<void>(`${environment.apiUrl}/groups/${groupId}`, { withCredentials: true });
    }
}
